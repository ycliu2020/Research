...
for ii = 1:2
    ...
    for jj = 1:2
        ...
        if strcmp(var, 'toa')
            ...
            if strcmp(var2, 'all')
                wv_lwkernel = ncread([filepath, 'TOA_allsky_r.nc'], 'wv_lwkernel');
                wv_swkernel = ncread([filepath, 'TOA_allsky_r.nc'], 'wv_swkernel');
                t_lwkernel = ncread([filepath, 'TOA_allsky_r.nc'], 't_lwkernel');
                ts_lwkernel = ncread([filepath, 'TOA_allsky_r.nc'], 'ts_lwkernel');
                alb_swkernel = ncread([filepath, 'TOA_allsky_r.nc'], 'alb_swkernel');
            else
                wv_lwkernel = ncread([filepath, 'TOA_clrsky_r.nc'], 'wv_lwkernel');
                wv_swkernel = ncread([filepath, 'TOA_clrsky_r.nc'], 'wv_swkernel');
                t_lwkernel = ncread([filepath, 'TOA_clrsky_r.nc'], 't_lwkernel');
                ts_lwkernel = ncread([filepath, 'TOA_clrsky_r.nc'], 'ts_lwkernel');
                alb_swkernel = ncread([filepath, 'TOA_clrsky_r.nc'], 'alb_swkernel');
            end
        ...
        elseif strcmp(var, 'sfc')
            ...
            if strcmp(var2, 'all')
                wv_lwkernel = ncread([filepath, 'SFC_allsky_r.nc'], 'wv_lwkernel');
                wv_swkernel = ncread([filepath, 'SFC_allsky_r.nc'], 'wv_swkernel');
                t_lwkernel = ncread([filepath, 'SFC_allsky_r.nc'], 't_lwkernel');
                ts_lwkernel = ncread([filepath, 'SFC_allsky_r.nc'], 'ts_lwkernel');
                alb_swkernel = ncread([filepath, 'SFC_allsky_r.nc'], 'alb_swkernel');
            else
                wv_lwkernel = ncread([filepath, 'SFC_clrsky_r.nc'], 'wv_lwkernel');
                wv_swkernel = ncread([filepath, 'SFC_clrsky_r.nc'], 'wv_swkernel');
                t_lwkernel = ncread([filepath, 'SFC_clrsky_r.nc'], 't_lwkernel');
                ts_lwkernel = ncread([filepath, 'SFC_clrsky_r.nc'], 'ts_lwkernel');
                alb_swkernel = ncread([filepath, 'SFC_clrsky_r.nc'], 'alb_swkernel');
            end
            ...
            for kk = 1:216
                ...
                t0Effect = squeeze(t_lwkernel(:, :, 1, mod(kk + 1, 12) + 1)) .* tsAnom(:, :, kk);
                tsEffect(:, :, kk) = ts_lwkernel(:, :, mod(kk + 1, 12) + 1) .* tsAnom(:, :, kk);
                albEffect(:, :, kk) = alb_swkernel(:, :, mod(kk + 1, 12) + 1) .* albAnom(:, :, kk) * 100;
            end
