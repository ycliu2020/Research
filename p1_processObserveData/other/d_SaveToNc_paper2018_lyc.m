...
filename1 = '/home/lyc/repeat_Kernel_experiment/testdata/EBAF_clear/sfc_all.nc';
...
% 2) dRx (time, lat, lon, budget, spectrum, sky, x): radiation anomaly, deseasoned and detrended
    ...
    dRx = NaN(5,2,3,2,144,72,216);                                        % deseasonalized and detrended
    dRx0 = NaN(5,2,3,2,144,72,216);                                        % only deseasonalized
    trend_Rx = NaN(5,2,3,2,144,72,12,2);
    p_Rx = NaN(5,2,3,2,144,72,12);
    cons_Rx = NaN(5,2,3,2,144,72,12);
    
    dRxs = NaN(5,2,3,2,144,72,216);                                        % deseasonalized and detrended
    dRxs0 = NaN(5,2,3,2,144,72,216);                                        % only deseasonalized
    trend_Rxs = NaN(5,2,3,2,144,72,12,2);
    p_Rxs = NaN(5,2,3,2,144,72,12);
    cons_Rxs = NaN(5,2,3,2,144,72,12);
    ...
    for ii = 1:2
        for zz = 1:2
            for jj = 1:3
                ...
                filename = strcat('/home/lyc/repeat_Kernel_experiment/testdata/EBAF_clear/',var,'_',var3,'.nc');
%                 filename = strcat('/data1/xieyan/EBAF_clear/',var,'_',var3,'.nc');
                
                tot0 = ncread(filename,strcat('dR',var2)); tot = NaN(size(tot0));
                T0 = ncread(filename,'dR_T'); T = NaN(size(T0));
                ta0 = ncread(filename,'dR_ta'); ta = NaN(size(ta0));
                ts0 = ncread(filename,'dR_ts'); ts = NaN(size(ts0));
                q0 = ncread(filename,strcat('dR',var2,'_q')); q = NaN(size(q0));
                a0 = ncread(filename,strcat('dR_alb')); a = NaN(size(a0));
                nonc0 = ncread(filename,strcat('dR',var2,'_noncloud')); nonc = NaN(size(nonc0));
                res0 = ncread(filename,strcat('dR',var2,'_res')); res = NaN(size(res0));
                if strcmp(var3,'all')
                    c0 = ncread(filename,strcat('dR',var2,'_c')); c = NaN(size(c0));
                ...
                % detrending
                trend_tot = zeros(nlon,nlat,12,2); trend_T = zeros(nlon,nlat,12,2); trend_q = zeros(nlon,nlat,12,2); trend_a = zeros(nlon,nlat,12,2); trend_c = zeros(nlon,nlat,12,2);
                trend_nonc = zeros(nlon,nlat,12,2); trend_diag = zeros(nlon,nlat,12,2); trend_res = zeros(nlon,nlat,12,2); trend_ta = zeros(nlon,nlat,12,2); trend_ts = zeros(nlon,nlat,12,2);
                
                p_tot = zeros(nlon,nlat,12); p_T = zeros(nlon,nlat,12); p_q = zeros(nlon,nlat,12); p_a = zeros(nlon,nlat,12); p_c = zeros(nlon,nlat,12);
                p_nonc = zeros(nlon,nlat,12); p_diag = zeros(nlon,nlat,12); p_res = zeros(nlon,nlat,12); p_ta = zeros(nlon,nlat,12); p_ts = zeros(nlon,nlat,12);
                
                cons_tot = zeros(nlon,nlat,12); cons_T = zeros(nlon,nlat,12); cons_q = zeros(nlon,nlat,12); cons_a = zeros(nlon,nlat,12); cons_c = zeros(nlon,nlat,12);
                cons_nonc = zeros(nlon,nlat,12); cons_diag = zeros(nlon,nlat,12); cons_res = zeros(nlon,nlat,12); cons_ta = zeros(nlon,nlat,12); cons_ts = zeros(nlon,nlat,12);
...
%% 1:Radiation Anomaly
ncid = netcdf.create('/home/lyc/repeat_Kernel_experiment/testdata/datasets_radAnom2018.nc','NC_WRITE');
% ncid = netcdf.create('/data1/xieyan/EBAF_clear/datasets_radAnom2018.nc','NC_WRITE');
...
dRx0_ID = netcdf.defVar(ncid,'dRx0','double',[dimidx dimidsky dimidspec dimidbudget dimidlon dimidlat dimidtime]);
R_ID = netcdf.defVar(ncid,'R','double',[dimidsky dimidspec dimidbudget dimidlon dimidlat dimidmonth]);
...
netcdf.putAtt(ncid,dRx0_ID,'long_name','Radiation Anomaly, Deseasonalized');
netcdf.putAtt(ncid,dRx0_ID,'units','W m-2');
netcdf.putAtt(ncid,R_ID,'long_name','Radiation Monthly Climatology');
netcdf.putAtt(ncid,R_ID,'units','W m-2');
...
netcdf.putVar(ncid,dRx0_ID,dRx0);
...
ncid = netcdf.create('/home/lyc/repeat_Kernel_experiment/testdata/datasets_radTrend2018.nc','NC_WRITE');
% ncid = netcdf.create('/data1/xieyan/EBAF_clear/datasets_radTrend2018.nc','NC_WRITE');
%Define the dimensions
dimidlon = netcdf.defDim(ncid,'longitude',nlon);
dimidlat = netcdf.defDim(ncid,'latitude',nlat);
dimidtime = netcdf.defDim(ncid,'time',ntime);
...
%Define the main variable ()
dRx_ID = netcdf.defVar(ncid,'dRx','double',[dimidx dimidsky dimidspec dimidbudget dimidlon dimidlat dimidtime]);
...
netcdf.putAtt(ncid,month_ID,'long_name','month');
netcdf.putAtt(ncid,month_ID,'units','Month (JFMAMJJASOND)');
netcdf.putAtt(ncid,dRx_ID,'long_name','Radiation Anomaly, Deseasonalized and Detrended');
netcdf.putAtt(ncid,dRx_ID,'units','W m-2');
...
%% 3:Supplementary file Radiation Anomaly
ncid = netcdf.create('/home/lyc/repeat_Kernel_experiment/testdata/datasets_radAnom2018_supp.nc','NC_WRITE');
% ncid = netcdf.create('/data1/xieyan/EBAF_clear/datasets_radAnom2018_supp.nc','NC_WRITE');
%Define the dimensions
dimidlon = netcdf.defDim(ncid,'longitude',nlon);
dimidlat = netcdf.defDim(ncid,'latitude',nlat);
dimidtime = netcdf.defDim(ncid,'time',ntime);
...
x_ID =netcdf.defVar(ncid,'x','int',dimidx);
%Define the main variable ()
dRxs0_ID = netcdf.defVar(ncid,'dRxs0','double',[dimidx dimidsky dimidspec dimidbudget dimidlon dimidlat dimidtime]);
%Define units for the dimension variables
netcdf.putAtt(ncid,longitude_ID,'standard_name','longitude');
netcdf.putAtt(ncid,latitude_ID,'standard_name','latitude');
netcdf.putAtt(ncid,time_ID,'long_name','time');
...
netcdf.putAtt(ncid,month_ID,'long_name','month');
netcdf.putAtt(ncid,month_ID,'units','Month (JFMAMJJASOND)');
netcdf.putAtt(ncid,dRxs0_ID,'long_name','Radiation Anomaly, Deseasonalized');
netcdf.putAtt(ncid,dRxs0_ID,'units','W m-2');
netcdf.putAtt(ncid,budget_ID,'standard_name','budget');
netcdf.putAtt(ncid,budget_ID,'units','TOA; SFC');
netcdf.putAtt(ncid,spec_ID,'standard_name','spectrum');
netcdf.putAtt(ncid,spec_ID,'units','NET; LW; SW');
...
netcdf.putVar(ncid,x_ID,five);
%Then store main variable
netcdf.putVar(ncid,dRxs0_ID,dRxs0);
%We're done, close the netcdf
netcdf.close(ncid)
...
ncid = netcdf.create('/home/lyc/repeat_Kernel_experiment/testdata/datasets_radTrend2018supp.nc','NC_WRITE');
% ncid = netcdf.create('/data1/xieyan/EBAF_clear/datasets_radTrend2018supp.nc','NC_WRITE');
%Define the dimensions
dimidlon = netcdf.defDim(ncid,'longitude',nlon);
dimidlat = netcdf.defDim(ncid,'latitude',nlat);
dimidtime = netcdf.defDim(ncid,'time',ntime);
...
coef_ID = netcdf.defVar(ncid,'coef','int',dimidcoef);
%Define the main variable ()
dRxs_ID = netcdf.defVar(ncid,'dRxs','double',[dimidx dimidsky dimidspec dimidbudget dimidlon dimidlat dimidtime]);
...
netcdf.putAtt(ncid,month_ID,'long_name','month');
netcdf.putAtt(ncid,month_ID,'units','Month (JFMAMJJASOND)');
netcdf.putAtt(ncid,dRxs_ID,'long_name','Radiation Anomaly, Deseasonalized and Detrended');
netcdf.putAtt(ncid,dRxs_ID,'units','W m-2');
...
netcdf.putVar(ncid,coef_ID,two);
%Then store main variable
netcdf.putVar(ncid,dRxs_ID,dRxs);
