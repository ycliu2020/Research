...
lonf = ncread('/data/pub/kernels_YiH/toa/RRTMG_wv_lw_toa_cld_highR.nc', 'lon'); nlonf = length(lonf);
latf = ncread('/data/pub/kernels_YiH/toa/RRTMG_wv_lw_toa_cld_highR.nc', 'lat'); nlatf = length(latf);
...
for ii = 1:2
    ...
    for jj = 1:2
        ...
        if strcmp(var, 'toa')
            ...
            filepath='/data/pub/kernels_YiH/toa/';
            dp = ncread([filepath, 'dp.nc'], 'dp');
            numdp = length(dp);

            if strcmp(var2, 'all')
                wv_lwkernel = ncread([filepath, 'RRTMG_wv_lw_toa_cld_highR.nc'], 'lwkernel');
                wv_swkernel = ncread([filepath, 'RRTMG_wv_sw_toa_cld_highR.nc'], 'swkernel');
                t_lwkernel = ncread([filepath, 'RRTMG_t_toa_cld_highR.nc'], 'lwkernel');
                ts_lwkernel = ncread([filepath, 'RRTMG_ts_toa_cld_highR.nc'], 'lwkernel');
                alb_swkernel = ncread([filepath, 'RRTMG_alb_toa_cld_highR.nc'], 'swkernel');
            else
                wv_lwkernel = ncread([filepath, 'RRTMG_wv_lw_toa_clr_highR.nc'], 'lwkernel');
                wv_swkernel = ncread([filepath, 'RRTMG_wv_sw_toa_clr_highR.nc'], 'swkernel');
                t_lwkernel = ncread([filepath, 'RRTMG_t_toa_clr_highR.nc'], 'lwkernel');
                ts_lwkernel = ncread([filepath, 'RRTMG_ts_toa_clr_highR.nc'], 'lwkernel');
                alb_swkernel = ncread([filepath, 'RRTMG_alb_toa_clr_highR.nc'], 'swkernel');
            end
            ...
            dp1_path = '/home/lyc/repeat_Kernel_experiment/testdata/ERAi/';
        ...
        elseif strcmp(var, 'sfc')
            ...
            filepath='/data/pub/kernels_YiH/surface/';
            dp = ncread([filepath, 'dp.nc'], 'dp');
            numdp = length(dp);

            if strcmp(var2, 'all')
                wv_lwkernel = ncread([filepath, 'RRTMG_wv_lw_surface_cld_highR.nc'], 'lwkernel');
                wv_swkernel = ncread([filepath, 'RRTMG_wv_sw_surface_cld_highR.nc'], 'swkernel');
                t_lwkernel = ncread([filepath, 'RRTMG_t_surface_cld_highR.nc'], 'lwkernel');
                ts_lwkernel = ncread([filepath, 'RRTMG_ts_surface_cld_highR.nc'], 'lwkernel');
                alb_swkernel = ncread([filepath, 'RRTMG_alb_surface_cld_highR.nc'], 'swkernel');
            else
                wv_lwkernel = ncread([filepath, 'RRTMG_wv_lw_surface_clr_highR.nc'], 'lwkernel');
                wv_swkernel = ncread([filepath, 'RRTMG_wv_sw_surface_clr_highR.nc'], 'swkernel');
                t_lwkernel = ncread([filepath, 'RRTMG_t_surface_clr_highR.nc'], 'lwkernel');
                ts_lwkernel = ncread([filepath, 'RRTMG_ts_surface_clr_highR.nc'], 'lwkernel');
                alb_swkernel = ncread([filepath, 'RRTMG_alb_surface_clr_highR.nc'], 'swkernel');
            end
        ...
        saveto = strcat(directory_name, 'kernel_accumulate_', upper(var), '_', var2, 'sky_r.nc');
