...
% note: units:/10a eE: trend*365*10
% Ts: K/10 a , rhs:W/m^(-2)/10 a
% color map: 16
% contain three database, two resolution(lon,lat, lath,lonh)
%
clc; clear;tic; %计时
inputpath_ERA = '/data1/liuyincheng/Observe-process/data_Tsradtrend/';
inputpath_Had_CER = '/data1/liuyincheng/Observe-process/data_Tsradtrend/';
...
action = {'Ts trend(K/10a)','LW down+SW trend(W\cdotm-2/10a)'};
...
% ts2(~maskworld_cp)=NaN;%mask land
...
% cc(1,1) = roundn(cc0(1,2),-2);pp(1,1) = pp0(1,2);
% cc(2,1) = roundn(cc1(1,2),-2);pp(2,1) = pp1(1,2);
...
% Compute correlation(seasonly)
    ...
    cc(1,1) = roundn(cc0(1,2),-2);pp(1,1) = pp0(1,2);
    cc(2,1) = roundn(cc1(1,2),-2);pp(2,1) = pp1(1,2);
    cc(3,1) = roundn(cc2(1,2),-2);pp(3,1) = pp2(1,2);
