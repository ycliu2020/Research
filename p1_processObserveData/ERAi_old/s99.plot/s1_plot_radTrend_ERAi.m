...
for toaSfc = 1:2
    ...
    for landSea = 1:2
        [mlabels, areaNum] = obsPlotParameters(inputLevel{toaSfc}, inputArea{landSea},'erai'); % plot parameters
        ...
        for p_1 = 1:2
            ...
            mPath.input = fullfile('/data1/liuyincheng/Observe-process', tLin.time{p_1}, '/ERAi/'); %/data1/liuyincheng/Observe-process/200207-201706
            mPath.Output = fullfile('/home/lyc/research/P02.Ts_change_research/figure/Observe/1.2/trend_radEffect_ERAi/', mlabels.level, tLin.time{p_1});
            % mPath.Output = fullfile(mPath.uniOutput, mlabels.fileN1);
            auto_mkdir(mPath.Output)
            dradTrendPath = [mPath.input, 'radEffect_trend/'];
            dRheatingPath = [mPath.input, 'anomaly_trend/'];
            load([dRheatingPath, 'trend_dtsdradERAi.mat'])%lat, lon, p_rhs, p_ts, time, trendm_drhs, trendm_dts, trendyr_drhs, trendyr_dts
            load([dradTrendPath, 'trend_dradEfect_all_ERAi.mat'])%latPlot, lonPlot, time, trendyr_dRsfc_alb, trendyr_dRsfc_cloud, trendyr_dRsfc_q, trendyr_dRsfc_realRad, trendyr_dRsfc_ta, trendyr_dRsfc_total, trendyr_dRsfc_total_c, trendyr_dRsfc_ts, trendyr_dRtoa_alb, trendyr_dRtoa_cloud, trendyr_dRtoa_q, trendyr_dRtoa_realRad, trendyr_dRtoa_ta, trendyr_dRtoa_total, trendyr_dRtoa_total_c, trendyr_dRtoa_ts
