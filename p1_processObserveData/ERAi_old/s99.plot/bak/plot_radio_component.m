...
component_label={'Ts','rhs','dR_t','dR_q','dR_cloud','dR_alb'};
...
trend_var_m(:,lat>60|lat<-60,:,:)=NaN;%划定纬度剔除两个极地
...
trend_var_y(:,lat>60|lat<-60,:,:)=NaN;%划定纬度剔除两个极地
...
%     end
title({[dataname{p1},' ',component{ii},unite{ii}];['(cc = ',num2str(cc(ii)),')']});% cc=',num2str(corr))
    hold on
