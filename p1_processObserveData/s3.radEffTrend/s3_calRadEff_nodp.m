...
dhus2 = zeros(144, 73, 24, 216);

for kk = 1:216
    dhus2(:, :, :, kk) = (log(q(:, :, :, kk)) - log(qClim(:, :, :, mod(kk + 1, 12) + 1))) .* tClim(:, :, :, mod(kk + 1, 12) + 1).^2 * Rv / Lv;
end
dhus2(isnan(dhus2)) = 0;
...
for ii = 1:2
    ...
    for jj = 1:2
        ...
        saveto = strcat(directory_name, 'eraiRadAnom_2018', upper(var), var2, 'sky_nodp.nc');
