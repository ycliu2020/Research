...
% LastEditTime : 2021-04-12 20:00:46
% LastEditors  : Please set LastEditors
% Description  : compare the hus computation of Angeline G and now applied
% FilePath     : /code/p1_processObserveData/ERA5/checkHusAndRcloud/ERA5_s3_calRadEff_dp_v2_Huangyi.m
%
%%---------------------------------------------------------
clear; clc; tic;
% pause(9999)
...
for p_1 = 4:4
    ...
    dws=dlnws(clim_ta);
    for yearNum = 1:ntime/12
        dhus2(:,:,:,(yearNum-1)*12+1:yearNum*12)=dhus(:,:,:,(yearNum-1)*12+1:yearNum*12).*dws;%.*clim_ta.^2 * Rv / Lv;
    end
    [dhus2, dhus2_clim] = monthlyAnomaly(144, 73, plevel, time, dhus2, startMonth); %[nlongitude,nlatitude,time,var,startmonth]
    for monNum = 1:ntime
        dhus2(:, :, :, monNum) = dhus2(:, :, :, monNum) .* clim_ta(:, :, :, mod(monNum + startMonth - 2, 12) + 1).^2 * Rv / Lv;
    end
...
function dws=dlnws(T)
    ...
    ws=T;
    ws1=ws;
    ws(T>=273.15)=pliq(T>=273.15);
    ws(T<273.15)=pice(T<273.15);

    ws1(T>=273.15)=pliq1(T>=273.15);
    ws1(T<273.15)=pice1(T<273.15);
