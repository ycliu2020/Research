...
% LastEditTime : 2020-11-10 21:29:19
% LastEditors  : LYC
% Description  : 同时画时间序列和相关性分布图
% FilePath     : /code/p1_processObserveData/ERA5/timeSeriAnalysis/ERA5_timeSeriANS_radCloseTest.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
% data path
dvarsPath = fullfile(ERA5Path, 'anomaly/'); %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/anomaly
dradEffectPath = fullfile(ERA5Path, 'radEffect/'); %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/radEffect/
...
dslhf = autoRegrid3(lon_k, lat_k, time, dslhf, lon_f, lat_f, time);
dsshf = autoRegrid3(lon_k, lat_k, time, dsshf, lon_f, lat_f, time);

dhFlux = -dslhf - dsshf; % LH+SH
...
varUsed(:, :, :, 1) = dR_net_cld_sfc;
varUsed(:, :, :, 2) = tsEffect + husEffect + taEffect + albEffect + dR_cloud_sfc;
varUsed(:, :, :, 3) = varUsed(:, :, :, 1) - varUsed(:, :, :, 2);
% dR_residual_clr_sfc+dhFlux;
% varUsed(:, :, :, 3) = taEffect;
% varUsed(:, :, :, 4) = albEffect;
% varUsed(:, :, :, 5) = dR_residual_cld_sfc-(dslhf+dsshf);
% varUsed(:, :, :, 6) = dR_residual_cld_toa;
% varUsed(:, :, :, 7) = dslhf.mat+dsshf;

% varUsed(:, :, :, 7) = dR_net_cld_sfc; %sum(varUsed(:,:,:,1:6),4);
...
% varUsed(:, :, :, 1) = tsEffect;
% varUsed(:, :, :, 2) = dRheating;
% varUsed(:, :, :, 3) = -dR_net_cld_sfc;
% varUsed(:, :, :, 4) = dhFlux;
% varUsed(:, :, :, 5) = tsEffect+dRheating-dR_net_cld_sfc;
