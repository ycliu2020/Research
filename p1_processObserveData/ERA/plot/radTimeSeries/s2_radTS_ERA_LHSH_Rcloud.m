...
% Description  :
% FilePath     : /code/p1_processObserveData/ERA/plot/radTimeSeries/s2_radTS_ERA_LHSH_Rcloud.m
%
%%---------------------------------------------------------
...
for exmNum = 5:5 % '200003-201802', '200207-201706', '200003-201402', '200001-201412', '198001-201412', '200003-202011'
    ...
    ntime = length(time);
    % load save one var
    load([figDataPath, 'regionalVarsRad_sfc_cld.mat']) % 'dR_cloudMask', 'dR_residualMask', 'dR_albMask', 'dR_husMask', 'dR_taMask', 'dR_tsMask''dhfssMask', 'dhflsMask', 'drhsMask', 'drhsKernMask', 'dlwUPMask', 'dlwUPRawDataMask'
    ...
    varUsed(:, :, :, 4, :)=varUsed(:, :, :, 6, :)+varUsed(:, :, :, 5, :);% dRcloud + Res
    disp('plz check the correction of res!')
    varUsed(:, :, :, 5, :)=varUsed(:, :, :, 1, :)-varUsed(:, :, :, 4, :);% 
...
function [] = plotPart(exmNum, detrendCode, timeYearly, varUsedYearly_weightMean2, sizeLon, sizeLat, sizeTime, sizeVar, sizeRegion)
    ...
    varNames = {['-(LH+SH)'], ['d', '\itR', '\rm_{WV}'], ['d', '\itR', '\rm_{Ta}'], ['d', '\itR', '\rm_{cld}', '\it+res'], ['\rm{Residual}'], ['d', '\itR', '\rm_{cld}']};
    unitName = {'W m^{-2}'};
    ...
    varColor = {'#F08212', 'none', 'none', '#C2162E', '#C5C1C1', 'none'}; %橘色 深蓝 浅蓝 浅绿 黑色 红色#C2162E
