...
sfcKernelPath =[kernelsPath,'surface/'];
plevel = ncread([sfcKernelPath, 'dp.nc'], 'plevel');
player = ncread([sfcKernelPath, 'dp.nc'], 'player');
dp_raw = ncread([sfcKernelPath, 'dp.nc'], 'dp');

% read lat lon
lon_k = ncread([kernelsPath,'toa/RRTMG_wv_lw_toa_cld_highR.nc'], 'lon'); nlonk = length(lon_k);
lat_k = ncread([kernelsPath,'toa/RRTMG_wv_lw_toa_cld_highR.nc'], 'lat'); nlatk = length(lat_k);
