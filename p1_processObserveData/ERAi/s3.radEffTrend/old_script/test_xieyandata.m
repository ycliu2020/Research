...
ncfilename1 = strcat(inputpath_component,'datasets_radTrend2018.nc');
ncfilename2 = strcat(inputpath_component,'datasets_radTrend2018supp.nc');
trend_Rx = ncread(ncfilename1,'trend_Rx');% (5,2,3,2,144,72,12,2) total, temperature, water vapor, albedo, cloud
trend_Rxs = ncread(ncfilename2,'trend_Rxs');% (5,2,3,2,144,72,12,2) non-cloud, diagnosed all, residual, ta, ts
% SFC/TOA condition(以下条件恒定:Sky=1(allsky),spectrum=1(net),ceof=1)
condition=1;% 1toa,2sfc

mon_trend_dR_ts=squeeze(squeeze(squeeze(squeeze(trend_Rxs(5,1,1,condition,:,:,:,1)))));
mon_trend_dR_cloud=squeeze(squeeze(squeeze(squeeze(trend_Rx(5,1,1,condition,:,:,:,1)))));
mon_trend_dR_t=squeeze(squeeze(squeeze(squeeze(trend_Rxs(4,1,1,condition,:,:,:,1)))));
mon_trend_dR_q=squeeze(squeeze(squeeze(squeeze(trend_Rx(3,1,1,condition,:,:,:,1)))));
mon_trend_dR_alb=squeeze(squeeze(squeeze(squeeze(trend_Rx(4,1,1,condition,:,:,:,1)))));

yr_trend_dR_ts=squeeze(mean(mon_trend_dR_ts,3));
yr_trend_dR_cloud=squeeze(mean(mon_trend_dR_cloud,3));
yr_trend_dR_t=squeeze(mean(mon_trend_dR_t,3));
yr_trend_dR_q=squeeze(mean(mon_trend_dR_q,3));
yr_trend_dR_alb=squeeze(mean(mon_trend_dR_alb,3));
...
% trend_var_s(:,lat>60|lat<-60,:,:)=NaN;%划定纬度剔除两个极地
