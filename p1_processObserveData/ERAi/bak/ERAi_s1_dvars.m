...
% LastEditTime : 2021-04-22 14:51:53
...
startT = cdftime2loc(temp_dir, formatOut, '1979-01');
...
% read vars
startLoc = [1, 1, startT]; count = [inf, inf, count]; stride = [1, 1, 1];
...
for p_1 = 1:5 % 1 mean 200003-201802
    regridPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{1});
    anomPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{2});
    anomTrendPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{3});
    kernelCalPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{4});
    radEffectPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{5});
    radEffectTrendPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{6});
