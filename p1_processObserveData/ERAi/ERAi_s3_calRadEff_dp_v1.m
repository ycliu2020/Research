...
% LastEditTime : 2021-05-06 17:52:34
% LastEditors  : Please set LastEditors
% Description  :
% FilePath     : /code/p1_processObserveData/ERAi/ERAi_s3_calRadEff_dp_v1.m
%
%%---------------------------------------------------------
clear; clc; tic;
% pause(9999)
...
for p_1 = 2:5 %'200003-201802', '200207-201706', '200003-201402','200001-201412','198001-201412','200003-202011'
    varsPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{1}); %rawdata
    dvarsPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{2}); %anomaly
    kernelCalPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{4}); % kernelCal
    radEffectPath = fullfile(obsPath, tLin.time{p_1}, 'ERAi', level.standVarPath{5}); %radEffect
    ...
    for sfcToaCldClr = 1:4%( 1sfc cld,2sfc clr,3toa cld,4toa clr)
        ...
        if sfcToaCldClr <= 2
            ...
            save([radEffectPath, saveradEffectName{sfcToaCldClr}], 'wvlwEffect', 'wvswEffect', 'tsEffect', 'albEffect', 'husEffect', ...
                'taEffect', 'taOnlyEffect', 'tasEffect', 'nonCloudEffect', 'nonCloudAndTsEffect');
        else
            tasEffect2 = autoRegrid3(lon_k, lat_k, time, tasEffect2, lon_f, lat_f, time);
            tasEffect1 = autoRegrid3(lon_k, lat_k, time, tasEffect1, lon_f, lat_f, time);
            % save the radEffect: dradEffect_sfc_cld.mat
            save([radEffectPath, 'global_vars.mat'], 'lon_f', 'lat_f', 'lon_k', 'lat_k', 'plev_k', 'time')
            save([radEffectPath, saveradEffectName{sfcToaCldClr}], 'wvlwEffect', 'wvswEffect', 'tsEffect', 'albEffect', 'husEffect', ...
                'taEffect', 'nonCloudEffect', 'nonCloudAndTsEffect');
        end
