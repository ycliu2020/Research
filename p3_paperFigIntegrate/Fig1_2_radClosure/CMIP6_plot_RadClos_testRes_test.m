...
% LastEditTime : 2021-04-25 11:14:17
% LastEditors  : Please set LastEditors
% Description  : 同时画时间序列和相关性分布图
% FilePath     : /code/p3_paperFigIntegrate/Fig1_2_radClosure/CMIP6_plot_RadClos_testRes_test.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 1:1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % model
    for mdlNum = 1:1%length(level.model2) %1:length(level.model2)
        % model path
        ...
        for esmNum = specificNum:specificNum %1:1 %length(esmName) % note that r1i1p1 sometime not the first folder
            ...
            % cld
            dRcld_sfc = zeros(144, 72, ntime, 3);
            dRcld_toa = zeros(144, 72, ntime, 3);
            load([dradEffectPath, 'model_dradEffect.mat']) %dR_net_cld, dR_net_clr, lw_net, readme_realradEfect, sw_net
            dRcld_sfc(:, :, :, 1) = dR_net_cld(:, :, :, 1);
            dRcld_toa(:, :, :, 1) = dR_net_cld(:, :, :, 2);
            clear dR_net_cld dR_net_clr
            dRcld_sfc(:, :, :, 2) = lw_net(:, :, :, 1);
            dRcld_toa(:, :, :, 2) = lw_net(:, :, :, 3);
            clear lw_net
            dRcld_sfc(:, :, :, 3) = sw_net(:, :, :, 1);
            dRcld_toa(:, :, :, 3) = sw_net(:, :, :, 3);
            clear sw_net
            ...
            % cld
            dRcld_sfcKern = zeros(144, 72, ntime, 3);
            dRcld_toaKern = zeros(144, 72, ntime, 3);

            load([dradEffectPath, 'dradEffect_sfc_cld.mat'], 'lw_nonCloudEffect', 'sw_nonCloudEffect', 'nonCloudEffect')
            dRcld_sfcKern(:, :, :, 1) = nonCloudEffect;
            dRcld_sfcKern(:, :, :, 2) = lw_nonCloudEffect;
            dRcld_sfcKern(:, :, :, 3) = sw_nonCloudEffect;

            load([dradEffectPath, 'dradEffect_toa_cld.mat'], 'lw_nonCloudEffect', 'sw_nonCloudEffect', 'nonCloudEffect')
            dRcld_toaKern(:, :, :, 1) = nonCloudEffect;
            dRcld_toaKern(:, :, :, 2) = lw_nonCloudEffect;
            dRcld_toaKern(:, :, :, 3) = sw_nonCloudEffect;
            ...
            for varNum = 1:3
                ...
                [dRcld_sfc(:, :, :, varNum), ~, ~] = maskArea(squeeze(dRcld_sfc(:, :, :, varNum)), lat_f, latRange, -latRange, areaStr);
            ...
            for varNum = 1:3
                ...
                for timeNum = 1:ntime
                    ...
                    dRcldMean_sfc(timeNum, varNum) = areaMeanLatWeight(squeeze(squeeze(dRcld_sfc(:, :, timeNum, varNum))), lat_f);
            ...
            meanarea(:, 1, :, 1) = dRclrMean_sfcKern;
            meanarea(:, 1, :, 2) = dRcldMean_sfc-dRclrMean_sfc+dRclrMean_sfcKern;
            ...
            for waveProp = 1:3 % total, short,long
                ...
                for sfcToa = 1:2 % sfc and toa
                    ...
                    hold on
                    lineWdth = 1.5;
                    plot(time, squeeze(meanarea(:, sfcToa, waveProp, 3)), 'color', '#FCF8F9', 'LineWidth', 1.5) %#808080
                    ...
                    legend({Radname{waveProp}, [Radname{waveProp}, '(kernel calc) cc =', num2str(cc(sfcToa, waveProp))], 'Residual'}, 'Fontsize', 9, 'Location', 'none', 'position', axPosit, 'NumColumns', 2); %'Location', 'best''FontWeight', 'bold',
            ...
            figureTitle = ['Radiation closure experiment: ',areaStr];
            ...
            figureName = ['Fig2_', level.model2{mdlNum}, '_', esmName{esmNum}, '_200003-201402_',areaStr,'_testRes'];
            saveFileName = [mPath.Output, '/', figureName];
