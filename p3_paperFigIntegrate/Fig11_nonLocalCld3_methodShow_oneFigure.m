%%---------------------------------------------------------
% Author       : LYC
% Date         : 2021-02-08 17:47:28
% LastEditTime : 2021-03-11 21:48:32
% LastEditors  : Please set LastEditors
% Description  :
% FilePath     : /code/p3_paperFigIntegrate/Fig11_nonLocalCld3_methodShow_oneFigure.m
%
%%---------------------------------------------------------
clear; clc; tic;
...
outPutPath = '/home/liuyc/Research/P02.Ts_change_research/figure/proj3_PaperFig/v0.0/Fig11_CMIP6_showNonLocalCldCalcProcess/a_oneFigure/';
...
for exmNum = 4:4%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    for mdlNum = 1:length(level.model2)
        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            ss = get(0, 'ScreenSize');
            h = figure('Position', [11 -58 624 1006]);
            set(h, 'Color', [1 1 1]);

            h1 = subplot_yc(3, 1, 1, 1);
            hold on
            ...
            yLabel = '\Delta\itT\rm_{dg} (K)';
            xLabel = 'Year';

            unitePlot(xdata, ydata, xLabel, yLabel, timeSer, 'on');
            text(0.9, 0.15, '(a)', 'FontName', 'Microsoft YaHei', 'FontWeight', 'bold', 'Fontsize', 16, 'units', 'normalized');
            ...
            xdata = xdata(40:end);
            ydata = ydata(40:end);
            xLabel = '\Sigma(\itT\rm-\itT\rm_{dg})';
            yLabel = '\itC\rm_{d}\cdot\Delta\itT\rm_{dg}';

            h2 = subplot_yc(3, 1, 2, 1);
            hold on

            unitePlot(xdata, ydata, xLabel, yLabel, timeSer, 'off');
            text(0.9, 0.15, '(b)', 'FontName', 'Microsoft YaHei', 'FontWeight', 'bold', 'Fontsize', 16, 'units', 'normalized');
            hold on
            ...
            lgd_size = 12;
            lgd = legend('Annual data', 'Fitted line', 'FontName', 'Microsoft YaHei', 'Fontsize', lgd_size, 'Location', 'northwest');
            lgd_inf = get(lgd);
            ...
            rsq = roundn(rsq, -3);
            k = roundn(k, -3);
            text_ant = {['R^2= ', num2str(rsq)], ['slope(\gamma)=', num2str(k)]};

            text(lgd_inf.Position(1) - 0.12, lgd_inf.Position(2) + 0.03, text_ant, 'FontName', 'Microsoft YaHei', 'Fontsize', lgd_size, 'units', 'normalized');
            hold on
            ...
            h3 = subplot_yc(3, 1, 3, 1);
            hold on

            unitePlot(xdata, ydata, xLabel, yLabel, timeSer, 'on');
            text(0.9, 0.15, '(c)', 'FontName', 'Microsoft YaHei', 'FontWeight', 'bold', 'Fontsize', 16, 'units', 'normalized');

            DeltaTsg_cld = DeltaTsg - Delta_dtnc;

            figName = [outPutPath, 'oneFig_ssp370_r1i1p1f1_', mdlName, '_Jim.eps'];
            export_fig(gcf, figName, '-r600', '-cmyk')
            close gcf
...
function [] = unitePlot(xdata, ydata, xLabel, yLabel, timeSer, axtickSwich)
    ...
    plot(xdata, ydata, 'o', 'MarkerEdgeColor', 'k', 'MarkerFaceColor', '#67C1EE', 'LineWidth', 1.5, 'MarkerSize', 7)
    ...
    minXdata = xdata(1) - 5;
    maxXdata = xdata(end) + 5;
    xlim([minXdata maxXdata])
    minYdata = min(ydata) - 0.05 * (max(ydata) - min(ydata));
    maxYdata = max(ydata) + 0.05 * (max(ydata) - min(ydata));
    ylim([minYdata maxYdata])
    xlabel(xLabel)
    ylabel(yLabel)
    ...
    xtick = get(gca, 'XTick');
    ytick = get(gca, 'YTick');
    line([minXdata, maxXdata], [maxYdata maxYdata], 'Color', 'k', 'LineWidth', 1.5)
    line([maxXdata, maxXdata], [maxYdata minYdata], 'Color', 'k', 'LineWidth', 1.5)
