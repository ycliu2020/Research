...
% LastEditTime : 2020-12-07 17:05:19
...
for exmNum = 1:2%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    for mdlNum = 1:length(level.model2)
        ...
        for esmNum = specificNum:specificNum%1:length(esmName)
            ...
            figurePath = [outPutPath, '/', figName, '.png'];
...
function [outPutPath] = plotFig(varUsed, time, lat_f, startMonth)
    ...
    varNames = {'LW_{\uparrow}', 'dRHeating', 'dRHeating kernel calc', 'ts RadEfect', 'q RadEfect', 'alb RadEfect'};
    ...
    varUsedTemp3 = squeeze(varUsedYearly_weightMean(:, 3));

    lineWdth = 2;
    plot(timeYearly, varUsedTemp1', 'color', '#4450A1', 'LineWidth', lineWdth)
    hold on
    plot(timeYearly, varUsedTemp2', 'color', '#C2162E', 'LineWidth', lineWdth)% 深红
    hold on
    plot(timeYearly, varUsedTemp3', 'color', '#F08212', 'LineWidth', lineWdth)% 橘黄
    hold on
    ...
    lgd = legend('LW_{\uparrow}', ['dRHeating, cc=', num2str(cc_weightMean{2})], ['dRHeating(kernel calc), cc=', num2str(cc_weightMean{3})], 'Fontsize', 10, 'FontWeight', 'bold', 'Location', 'northwest', 'NumColumns', 2);
