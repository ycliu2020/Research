...
% LastEditTime : 2021-06-23 17:47:22
...
for exmNum = [1, 2, 4] %1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    % mask CHN US EUR
    [drhsMask, drhsKernMask, dlwUPKernMask, dlwUPRawDataMask, dhfssMask, dhflsMask] = maskMMEERA(lat_f, drhs, drhsKern, -tsEffect, dlwUPRawData, dhfss, dhfls);
...
function [drhsMask, drhsKernMask, dlwUPKernMask, dlwUPRawDataMask, dhfssMask, dhflsMask] = maskMMEERA(lat_f, drhs, drhsKern, ts_Effect, dlwUPRawData, dhfss, dhfls)
    % mask fun
    ...
    [dlwUPKernMask.world, ~, ~] = maskArea(ts_Effect, lat_f, latRange, -latRange, 'world');
    ...
    % mask coast
    [drhsMask.world] = maskArea_coast(drhsMask.world, lat_f, latRange, -latRange, 'world');
    [drhsKernMask.world] = maskArea_coast(drhsKernMask.world, lat_f, latRange, -latRange, 'world');
    [dlwUPKernMask.world] = maskArea_coast(dlwUPKernMask.world, lat_f, latRange, -latRange, 'world');
    [dlwUPRawDataMask.world] = maskArea_coast(dlwUPRawDataMask.world, lat_f, latRange, -latRange, 'world');
    [dhfssMask.world] = maskArea_coast(dhfssMask.world, lat_f, latRange, -latRange, 'world');
    [dhflsMask.world] = maskArea_coast(dhflsMask.world, lat_f, latRange, -latRange, 'world');

    % china
    [drhsMask.CHN, ~, ~] = maskArea(drhsMask.world, lat_f, latRange, -latRange, 'china');
    [drhsKernMask.CHN, ~, ~] = maskArea(drhsKernMask.world, lat_f, latRange, -latRange, 'china');
    [dlwUPKernMask.CHN, ~, ~] = maskArea(dlwUPKernMask.world, lat_f, latRange, -latRange, 'china');
    [dlwUPRawDataMask.CHN, ~, ~] = maskArea(dlwUPRawDataMask.world, lat_f, latRange, -latRange, 'china');
    [dhfssMask.CHN, ~, ~] = maskArea(dhfssMask.world, lat_f, latRange, -latRange, 'china');
    [dhflsMask.CHN, ~, ~] = maskArea(dhflsMask.world, lat_f, latRange, -latRange, 'china');

    % china east
    [drhsMask.CHNeast, ~, ~] = maskArea(drhsMask.world, lat_f, latRange, -latRange, 'china east');
    [drhsKernMask.CHNeast, ~, ~] = maskArea(drhsKernMask.world, lat_f, latRange, -latRange, 'china east');
    [dlwUPKernMask.CHNeast, ~, ~] = maskArea(dlwUPKernMask.world, lat_f, latRange, -latRange, 'china east');
    [dlwUPRawDataMask.CHNeast, ~, ~] = maskArea(dlwUPRawDataMask.world, lat_f, latRange, -latRange, 'china east');
    [dhfssMask.CHNeast, ~, ~] = maskArea(dhfssMask.world, lat_f, latRange, -latRange, 'china east');
    [dhflsMask.CHNeast, ~, ~] = maskArea(dhflsMask.world, lat_f, latRange, -latRange, 'china east');

    % US east
    [drhsMask.USeast, ~, ~] = maskArea(drhsMask.world, lat_f, latRange, -latRange, 'USA east');
    [drhsKernMask.USeast, ~, ~] = maskArea(drhsKernMask.world, lat_f, latRange, -latRange, 'USA east');
    [dlwUPKernMask.USeast, ~, ~] = maskArea(dlwUPKernMask.world, lat_f, latRange, -latRange, 'USA east');
    [dlwUPRawDataMask.USeast, ~, ~] = maskArea(dlwUPRawDataMask.world, lat_f, latRange, -latRange, 'USA east');
    [dhfssMask.USeast, ~, ~] = maskArea(dhfssMask.world, lat_f, latRange, -latRange, 'USA east');
    [dhflsMask.USeast, ~, ~] = maskArea(dhflsMask.world, lat_f, latRange, -latRange, 'USA east');

    % EUR west
    [drhsMask.EURwest, ~, ~] = maskArea(drhsMask.world, lat_f, latRange, -latRange, 'EUR west');
    [drhsKernMask.EURwest, ~, ~] = maskArea(drhsKernMask.world, lat_f, latRange, -latRange, 'EUR west');
    [dlwUPKernMask.EURwest, ~, ~] = maskArea(dlwUPKernMask.world, lat_f, latRange, -latRange, 'EUR west');
    [dlwUPRawDataMask.EURwest, ~, ~] = maskArea(dlwUPRawDataMask.world, lat_f, latRange, -latRange, 'EUR west');
    [dhfssMask.EURwest, ~, ~] = maskArea(dhfssMask.world, lat_f, latRange, -latRange, 'EUR west');
    [dhflsMask.EURwest, ~, ~] = maskArea(dhflsMask.world, lat_f, latRange, -latRange, 'EUR west');
