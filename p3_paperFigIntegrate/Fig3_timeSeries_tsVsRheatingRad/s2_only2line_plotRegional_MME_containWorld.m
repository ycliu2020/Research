...
% LastEditTime : 2021-04-21 09:22:09
% LastEditors  : Please set LastEditors
% Description  : MME result of 时间序列图
% FilePath     : /code/p3_paperFigIntegrate/Fig3_timeSeries_tsVsRheatingRad/s2_only2line_plotRegional_MME_containWorld.m
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% experiment
for exmNum_MME = 2:2 %1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    % ERA5 number
    ...
    varibalCode = {'drhsMask', 'dlwUPMask'};
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% plot Part
    radName = {['d', '\itR', '\rm_{heating} cc='], ['-d', '\itK', '\rm_{Ts}','\cdot\Delta','\itT','\rm_s']};
    unitName = {'W m^{-2}'};
    dataName = {'ERA5', 'AMIP'};
    ...
    for regionNum = 1:4
        ...
        for dataNum = 1:2 % ERA5 or CMIP6 Data
            ...
            plot(timeYearly, squeeze(varUsedYearly_weightMean(:, 1, regionNum, dataNum)), 'color', '#BC144A', 'LineWidth', lineWdth) % 真实值值
            plot(timeYearly, squeeze(varUsedYearly_weightMean(:, 2, regionNum, dataNum)), 'color', '#30388B', 'LineWidth', lineWdth) % 长波向上辐射
            ...
            legend({[radName{1}, num2str(cc_weightMean{1, regionNum, dataNum})], radName{2}}, 'FontName', 'Arial', 'Fontsize', 9, 'Location', 'none', 'position', axPosit, 'NumColumns', 2); %'Location', 'best''FontWeight', 'bold',
