...
% Description  :
% FilePath     : /code/p3_paperFigIntegrate/Fig3_timeSeries_tsVsRheatingRad/Fig_test/timeSeries_LHSH_chinaEast.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 2:2
    ...
    for mdlNum = 1:length(level.model2) %1:length(level.model2)
        ...
        for esmNum = specificNum:specificNum %1:1 %length(esmName) % note that r1i1p1 sometime not the first folder
            ...
            for regionNum = 1:sizeRegion
                ...
                for varNum = 1:sizeVar
                    ...
                    for timeNum = 1:sizeTime
                        varUsed_weightMean(timeNum, varNum, regionNum) = areaMeanLatWeight(varUsed(:, :, timeNum, varNum, regionNum), lat_f);
                    end
            ...
            unitName = {'W m^{-2}'};
            regionName = {['Eastern CHN'], ['Western CHN'], ['Eastern US']};
            ...
            for varNum = 1:4 % LH SH LH+SH
                ...
                for regionNum = 1:3 % 'CHNeast', 'world'
                    ...
                    hold on
                    lineWdth = 1.5;
                    plot(time, squeeze(varUsed_weightMean(:, varNum, regionNum)), 'color', '#BC144A', 'LineWidth', lineWdth) % 真实值值
                    plot(time, zeroLine', 'k--', 'LineWidth', 1.5)
