...
for exmNum = 2:4%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    for mdlNum = 1:length(level.model2)% model number
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % ensemble member
        count_esmName = length(esmName);

        parfor esmNum = 1:count_esmName
            esmFun(mdlPath, esmName{esmNum, 1}, alb_names, exmNum, mdlNum);
        end
...
function [] = esmFun(mdlPath, esmName1, alb_names, exmNum, mdlNum)
    %myFun - Description
    %
    ...
    esmPath = fullfile(mdlPath, esmName1);
    eval(['cd ', esmPath]);
    % outputpath and make it
    outPathName{1} = fullfile(outPath, level.time1{exmNum}, level.model2{mdlNum}, esmName1, level.process3{1}); %'model/ensemble member/rawdata_regrid/'
    outPathName{2} = fullfile(outPath, level.time1{exmNum}, level.model2{mdlNum}, esmName1, level.process3{2}); %'model/ensemble member/anomaly/'
    ...
    disp([esmName1, ' ensemble is done!'])
end
