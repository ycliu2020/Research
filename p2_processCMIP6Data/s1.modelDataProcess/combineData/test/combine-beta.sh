...
#===========================================================================
#
drc_in='/data1/liuyincheng/CMIP6-mirror/amip-hist/CESM2/hus/' # Directory of input files
var=('hus')                 # Variables
rlm='Amon'                         # Realm
xpt=( 'amip-hist' )                # Experiment ( could be more )

# for 0 in {0..0}; do            # Loop over two variables
# Names of all the models (ls [get file names];
#  cut [get model names];
#  sort; uniq [remove duplicates]; awk [print])
mdl_set=$( ls ${drc_in}${var[0]}_${rlm}_*_${xpt[0]}_*.nc | cut -d '_' -f 3 | sort | uniq -c | awk '{print $2}' )
# Number of models (echo [print contents]; wc [count])
mdl_nbr=$( echo ${mdl_set} | wc -w )
echo "=============================="
echo "There are" ${mdl_nbr} "models for" ${var[0]}.

for mdl in ${mdl_set}; do	        # Loop over models
    # Names of all the ensemble members
    nsm_set=$( ls ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_*.nc | \
    ...
    for nsm in ${nsm_set}; do	      # Loop over ensemble members
        ...
        fl_nbr=$( ls ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_*.nc \
        | wc -w )
        ...
        yyyymm_str=$( ls ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_*.nc\
        | sed -n '1p' | cut -d '_' -f 7 | cut -d '-' -f 1 )
        # Ending date of data
        #   (sed [the name of the last file includes the ending date])
        yyyymm_end=$( ls ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_*.nc\
        | sed -n "${fl_nbr}p" | cut -d '_' -f 7 | cut -d '-' -f 2 )
        
        # Concatenate one ensemble member files
        #   into one along the record dimension (now is time)
        ncrcat -O ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_*.nc \
        ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_gn_${yyyymm_str}-${yyyymm_end}
        
        # # Options:Remove useless files(rawdata) 
        # rm ${drc_in}${var[0]}_${rlm}_${mdl}_${xpt[0]}_${nsm}_gn_!(${yyyymm_str}-${yyyymm_end})
    done
done
# done