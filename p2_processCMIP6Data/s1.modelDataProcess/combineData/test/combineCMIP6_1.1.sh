...
for exp in ${exp_set}; do
    ...
    for tab_id in ${tab_set}; do
        for var_id in $(eval "echo {0..$jj}"); do            # Loop over two variables
            ...
            mdl_set=$( ls ${loc}${var_set[var_id]}_${tab_id}_*_${exp}_*.nc | \
            ...
            for mdl in ${mdl_set}; do	        # Loop over models
                ...
                esm_set=$( ls ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_*.nc | \
                ...
                for esm in ${esm_set}; do	      # Loop over ensemble members
                    ...
                    grd_set=$( ls ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_*.nc | \
                    ...
                    for grd in ${grd_set}; do	      # grid label
                        ...
                        fl_nbr=$( ls ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_*.nc \
                        | wc -w )
                        ...
                        yyyymm_str=$( ls ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_*.nc\
                        | sed -n '1p' | cut -d '_' -f 7 | cut -d '-' -f 1 )
                        # Ending date of data
                        #   (sed [the name of the last file includes the ending date])
                        yyyymm_end=$( ls ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_*.nc\
                        | sed -n "${fl_nbr}p" | cut -d '_' -f 7 | cut -d '-' -f 2 )
                        
                        # Concatenate one ensemble member files
                        #   into one along the record dimension (now is time)
                        ncrcat -O ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_*.nc \
                        ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_${yyyymm_str}-${yyyymm_end}
                        
                        # Remove useless files
                        rm ${loc}${var_set[var_id]}_${tab_id}_${mdl}_${exp}_${esm}_${grd}_!(${yyyymm_str}-${yyyymm_end})
