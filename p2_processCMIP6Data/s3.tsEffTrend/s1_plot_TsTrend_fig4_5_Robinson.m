        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            for varNum = 1:f_row * f_col
                ...
                if plotCol == 1 && plotRow == f_row
                    m_grid('linestyle', 'none','tickdir', 'out', 'linewi', 1.5, 'xtick', [60 180 300]);
                elseif plotCol == 1 && plotRow ~= f_row
                    m_grid('linestyle', 'none','tickdir', 'out', 'linewi', 1.5, 'xtick', [])
                elseif plotCol ~= 1 && plotRow == f_row
                    m_grid('linestyle', 'none','tickdir', 'out', 'linewi', 1.5, 'xtick', [60 180 300], 'ytick', [])
                elseif plotCol ~= 1 && plotRow ~= f_row
                    m_grid('linestyle', 'none','tickdir', 'out', 'linewi', 1.5, 'xtick', [], 'ytick', []);
                end
            ...
            figPath = [mPath.Output, '/', figName, '.pdf'];
            saveas(gcf, figPath)
