...
% LastEditTime : 2020-06-18 14:23:55
% LastEditors  : LYC
% Description  : 
% FilePath     : /code/p2_processCMIP6Data/s99.plot/s2p2_plotEachBars_nonLocalCld2.m
%  
%%---------------------------------------------------------
...
[mlabels, areaNum] = plotParameters('sfc', 'land', 'nonLocalCld2'); % plot parameters mlabels.level(SFC/TOA)
...
for p_1 = p1_left:p1_right
    ...
    mPath.uniOutput = fullfile('/home/liuyc/Research/P02.Ts_change_research/figure/02.cmip6Result/nonLocalCld2/',['dRTs_',lower(mlabels.level)], level.time1{p_1});
    ...
    for level1 = p2_left:p2_right
        % load data
        varsPath = [mPath.input, level.model2{level1}, '/', level.process3{1}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/rawdata
        dvarsPath = [mPath.input, level.model2{level1}, '/', level.process3{2}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/anomaly
        dvarsTrendPath = [mPath.input, level.model2{level1}, '/', level.process3{3}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/anomaly_trend
        kernelPath = [mPath.input, level.model2{level1}, '/', level.process3{5}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/kernelsCal
        dradEffectPath = [mPath.input, level.model2{level1}, '/', level.process3{6}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/radEffect/
        dradTrendPath = [mPath.input, level.model2{level1}, '/', level.process3{7}]; %/data1/liuyincheng/cmip6-process/amip_1980-2014/CESM2/radEffect_trend
        dnonLocalCldPath = [mPath.input, level.model2{level1}, '/', level.process3{8}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/non_localCld/
        vsTsEffectTrendPath = [mPath.input, level.model2{level1}, '/', level.process3{10}]; %/data1/liuyincheng/cmip6-process/2000-2014/MRI-ESM2-0/vsTsEffect_trend/
        ...
        ss = get(0, 'ScreenSize'); 
        h = figure('Position', [ss(4) / 2 ss(3) / 35 ss(3) * 3/4.5 ss(4) * 4/5]); 
        % set(h,'visible','off');
        % clf reset;
        set(h, 'Color', [1 1 1]);
