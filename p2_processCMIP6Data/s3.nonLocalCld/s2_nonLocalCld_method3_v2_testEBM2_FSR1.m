...
% LastEditTime : 2020-09-04 15:35:02
% LastEditors  : LYC
% Description  : 主要采用了线性假设的计算方式计算gamma项 修正计算非云致温度变化实验可行性分析 feasibility study report
% FilePath     : /code/p2_processCMIP6Data/s3.nonLocalCld/s2_nonLocalCld_method3_v2_testEBM2_FSR1.m
%
%%---------------------------------------------------------
    ...
    for mdlNum = 1:1%length(level.model2)
        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            for gridNum = 2:sum_timeExamp+1 % 循环每一个格点
                timeSeries = startY:timeExamp(gridNum-1);
        
                Seri_RnetTOA = gblM_R_netTOA(1:timeExamp(gridNum-1) - startY+1);
                Seri_dts = dts_ctrl(1:timeExamp(gridNum-1) - startY+1);
        
                %% cal Delta ts
                Delta_ts(gridNum) = autoCalDeltax(Seri_dts, timeSeries);
                integral_ts(gridNum) = sum(Seri_dts);
                integral_RnetTOA(gridNum) = sum(Seri_RnetTOA);
            end
        
            Delta_td = (integral_RnetTOA - C_ref .* Delta_ts) ./ Cd_ref;
            dtd_ctrl = Delta_td + b_td0;
            dtd_ctrl(2:end + 1) = dtd_ctrl; dtd_ctrl(1) = b_td0;
