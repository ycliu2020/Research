...
for p_1 = 1:4%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    for level1 = 1:length(level.model2)
        ...
        % load([radEffectPath, 'dradEfect_sfc_cld.mat'])%'wvlwEffect', 'wvswEffect', 'tsEffect',  'albEffect', 'husEffect', 'taEffect', 'tasEffect',taOnlyEffect',tasEffect2', 'taOnlyEffect2','realRadEffect'
        ...
        [trendm_dRsfc_realRad, trends_dRsfc_realRad, trendyr_dRsfc_realRad, ~, ~] = autoCalTrend(realRadEffect, nlon, nlat, time, startmonth);
