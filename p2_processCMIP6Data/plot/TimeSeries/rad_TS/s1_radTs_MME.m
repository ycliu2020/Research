...
% LastEditTime : 2021-05-12 15:09:43
% LastEditors  : Please set LastEditors
% Description  : 同时画时间序列和相关性分布图
% FilePath     : /code/p2_processCMIP6Data/plot/TimeSeries/rad_TS/s1_radTs_MME.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 1:1 %1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    figDataPath = fullfile(MMEPath, level.process3{11});
    ...
    load([figDataPath, 'regionalVarsRad_sfc_cld.mat']) % 'dR_cloudMask', 'dR_residualMask', 'dR_albMask', 'dR_husMask', 'dR_taMask', 'dR_tsMask' .world .CHNeast .EURwest .USeast
    ...
    varibalCode = {'dR_tsMask', 'dR_husMask', 'dR_taMask', ...
                    'dR_albMask', 'dR_residualMask', 'dR_cloudMask', ...
                    'dhfssMask', 'dhflsMask','drhsMask',...
                    'drhsKernMask','dlwUPRawDataMask'};
    ...
    % varUsed(:,:,:,5,:)=varUsed(:,:,:,5,:)-varUsed(:,:,:,7,:)-varUsed(:,:,:,8,:)+(-varUsed(:,:,:,1,:)+varUsed(:,:,:,2,:)+varUsed(:,:,:,3,:)+varUsed(:,:,:,4,:)+varUsed(:,:,:,6,:));% Rnet-Rvars-LH-SH-varUsed(:,:,:,7,:)-varUsed(:,:,:,8,:)
    % varUsed(:, :, :, 5, :) = varUsed(:, :, :, 1, :) - varUsed(:, :, :, 2, :) - varUsed(:, :, :, 3, :) - varUsed(:, :, :, 4, :); % Rts-Rvars
    % varUsed(:,:,:,5,:)=varUsed(:,:,:,1,:)-varUsed(:,:,:,2,:)-varUsed(:,:,:,3,:)-varUsed(:,:,:,4,:)-varUsed(:,:,:,6,:)-varUsed(:,:,:,7,:)-varUsed(:,:,:,8,:);% Rts-Rvars-LH-SH
    varUsed(:,:,:,5,:)=varUsed(:,:,:,10,:)-varUsed(:,:,:,9,:);
    sizeVar = 6;
    % global annual mean
    ...
    title_txt = {['Level:', num2str(toaSfc{2})], ['Data: ', level.time1{exmNum}(6:end - 11), ' ', level.time1{exmNum}(end - 9:end - 1)], ['Method: ', detrendSwich{detrendCode}]};
...
% figName = ['MME1_', level.time1{exmNum}(6:end - 1), '_', detrendSwich{detrendCode}];
...
% function varUsedWeightMean_Yearly = glbAnlMean(varUsed, sizeLon, sizeLat, sizeTime, sizeVar, sizeRegion)

function [] = plotPart(exmNum, detrendCode, timeYearly, varUsedYearly_weightMean2, sizeLon, sizeLat, sizeTime, sizeVar, sizeRegion)
    run '/home/liuyc/lib/tools/matlab/myTools/autoScript/preLoadVar.m'

    varNames = {['-d', '\itR', '\rm_{Ts}'], ['d', '\itR', '\rm_{WV}'], ['d', '\itR', '\rm_{Ta}'], ['d', '\itR', '\rm_{Alb}'], ['\rm{Residual}'], ['d', '\itR', '\rm_{cld}']};
    unitName = {'W m^{-2}'};
