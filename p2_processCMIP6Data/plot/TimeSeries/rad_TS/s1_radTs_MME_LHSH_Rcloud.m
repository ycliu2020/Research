...
% FilePath     : /code/p2_processCMIP6Data/plot/radTimeSeries/s1_radTs_MME_LHSH_Rcloud.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 4:4 %1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    % varUsed(:,:,:,5,:)=varUsed(:,:,:,5,:)+(varUsed(:,:,:,1,:)+varUsed(:,:,:,2,:)+varUsed(:,:,:,3,:)+varUsed(:,:,:,4,:)+varUsed(:,:,:,6,:));% Rnet-Rvars-LH-SH-varUsed(:,:,:,7,:)-varUsed(:,:,:,8,:)
    ...
    varUsed(:, :, :, 1, :)=varUsed(:, :, :, 7, :) + varUsed(:, :, :, 8, :);
    varUsed(:, :, :, 5, :)=varUsed(:, :, :, 1, :)-varUsed(:, :, :, 6, :);
...
function [] = plotPart(exmNum, detrendCode, timeYearly, varUsedYearly_weightMean2, sizeLon, sizeLat, sizeTime, sizeVar, sizeRegion)
    ...
    varNames = {['-(LH+SH)'], ['d', '\itR', '\rm_{WV}'], ['d', '\itR', '\rm_{Ta}'], ['d', '\itR', '\rm_{Alb}'], ['\rm{Residual}'], ['d', '\itR', '\rm_{cld}']};
    unitName = {'W m^{-2}'};
    ...
    varColor = {'#F08212', 'none', 'none', 'none', '#C5C1C1', '#C2162E'}; %橘色 深蓝 浅蓝 浅绿 黑色 红色
