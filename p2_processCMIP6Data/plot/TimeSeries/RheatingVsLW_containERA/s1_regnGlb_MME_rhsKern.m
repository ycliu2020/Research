...
% LastEditTime : 2021-05-11 14:48:21
% LastEditors  : Please set LastEditors
% Description  : MME result of 时间序列图
% FilePath     : /code/p2_processCMIP6Data/plot/TimeSeries/RheatingVsLW_containERA/s1_regnGlb_MME_rhsKern.m
...
for exmNum = 1:2 %1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    varibalCode = {'drhsKernMask', 'dlwUPKernMask'};
    ...
    load([ERA5figDataPath, 'regionalVarsRad_sfc_cld.mat']) % dlwUPMask, drhsKernMask, drhsMask  .world .CHNeast  .EURwest .USeast
    ...
    figName = [MMEType,'VsERA5_regnGlb_rhsKern_', tLin.time{exmNum_ERA5}];
...
function [] = plotPart(exmNum, timeType, time_plot, varUsed_plot, cc_weightMean, sizeLon, sizeLat, sizeTime, sizeVar, sizeRegion, sizeData)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% plot Part
    radName = {['d', '\itR', '\rm_{heating,Kern} cc='], ['-', '\itK', '\rm_{Ts}', '\cdot\Delta', '\itT', '\rm_s'], ['Residual']};
    unitName = {'W m^{-2}'};
    dataName = {'ERA5', 'AMIP'};
