%%---------------------------------------------------------
% Author       : LYC
% Date         : 2020-10-29 20:13:41
% LastEditTime : 2020-11-02 17:30:02
% LastEditors  : LYC
% Description  : Test whether RHeatiung caled by kernels and RHeatiung caled by model are consisten
% FilePath     : /code/p2_processCMIP6Data/s2.radEffTrend/contribAnalysis/test_RHeating.m
...
for exmNum = 1:1%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % model
    for mdlNum = 11:11%length(level.model2)
        % model path
        ...
        for esmNum = specificNum:specificNum%1:length(esmName)
            ...
            dRes_Rheating = -dR_netSfc + dR_residual_cld_sfc;

            varUsed = zeros(nlonf, nlatf, ntime, 2);
            varUsed(:, :, :, 1) = -tsEffect-(mainEffect+dR_cloud_sfc);%mainEffect+dR_cloud_sfc;%mainEffect+dR_cloud_sfc;  tsEffect-mainEffect-dR_cloud_sfc;
            varUsed(:, :, :, 2) = dR_cloud_sfc;% drhs;dR_cloud_sfc
            % varUsed(:, :, :, 3) =-tsEffect;% drhs;dR_cloud_sfc
            varNames = {'dResiual', 'dRcloud', 'dResiual', 'cld RadEfect','dRHeating(kernel cal)', 'dRHeating','dRes', 'cld RadEfect', 'ta RadEfect', 'ts RadEfect', 'q RadEfect', 'alb RadEfect'};%', 
            ...
            for  varNum = 1:sizeVar
                [varUsedYearly(:, :, :, varNum), ~, ~] = maskArea(varUsedYearly(:, :, :, varNum), lat_f, latRange, -latRange, areaNum);
            end
            ...
            if strcmp(varNames{1},'dRes')==1
                varUsedYearly_weightMean(:,1)=varUsedYearly_weightMean(:,3)-varUsedYearly_weightMean(:,1); 
            end
            ...
            ylabel(yLabel{2})
            % set y axes
            maxVar1=max(max(abs(varUsedTemp1)));
            maxVar2=max(max(abs(varUsedTemp2)));
            ymax = ceil(max(maxVar1, maxVar2));
            ...
            title_txt = {['Level:', num2str(toaSfc{2}), ', Era: ', level.time1{exmNum}(1:end - 1)], ['Model:', level.model2{mdlNum} ', Ensemble: ', esmName{esmNum}], ['Area: ', areaNum, ' ', varNames{1}, ' & ', varNames{2}, ' cc=', num2str(cc_weightMean{2})]};
            ...
            lgd = legend(varNames{1}, varNames{2}, 'Fontsize', 8, 'Location', 'northwest');
