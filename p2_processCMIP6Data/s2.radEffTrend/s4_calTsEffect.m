...
% LastEditTime : 2020-06-26 11:19:31
% LastEditors  : LYC
% Description  : cal dRvars effect on Ts : according Ts=dRx/Kts
% FilePath     : /code/p2_processCMIP6Data/s3.radEffTrend/dp_s4_calTsEffect.m
...
for p_1 = [2 4]%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    for level1 = 1:length(level.model2)
        ...
        for esmNum = 1:length(esmName)
            ...
            nlat = length(lat); nlon = length(lon); ntime = length(time.date);
            startmonth = 1;
            ...
            for skyLevel = 1:2% 1 mean toa, 2 mean sfc\
                ...
                % cal trend
                [trendm_dTs_cld, trends_dTs_cld, trendyr_dTs_cld, ~, ~] = autoCalTrend(dTs_cloud, nlon, nlat, time.date, startmonth);
                [trendm_dTs_hus, trends_dTs_hus, trendyr_dTs_hus, ~, ~] = autoCalTrend(dTs_hus, nlon, nlat, time.date, startmonth);
                [trendm_dTs_ta, trends_dTs_ta, trendyr_dTs_ta, ~, ~] = autoCalTrend(dTs_ta, nlon, nlat, time.date, startmonth);
                [trendm_dTs_alb, trends_dTs_alb, trendyr_dTs_alb, ~, ~] = autoCalTrend(dTs_alb, nlon, nlat, time.date, startmonth);
                % save
