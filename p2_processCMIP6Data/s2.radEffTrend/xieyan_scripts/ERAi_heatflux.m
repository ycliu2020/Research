...
clear
filepath = '/data1/xieyan/ERAi/Heat_Flux/';
...
for ii = 0:18
    ...
    temp5 = ncread(filename,'time');                                      % hours since 1900-01-01 00:00:00.0
        ...
        lahf0(:,:,ii*12+1:(ii+1)*12) = temp1(:,:,2:2:24);
        sehf0(:,:,ii*12+1:(ii+1)*12) = temp2(:,:,2:2:24);
        slwr0(:,:,ii*12+1:(ii+1)*12) = temp3(:,:,2:2:24);
        slwrd0(:,:,ii*12+1:(ii+1)*12) = temp4(:,:,2:2:24);
        time0(ii*12+1:(ii+1)*12,1) = temp5(2:2:24);
end
...
%% Save to nc
saveto = strcat(filepath,'ERAi_hfAnom.nc');
