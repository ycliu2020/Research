...
for exmNum = exmStart:exmEnd
    ...
    mPath.uniOutput = fullfile('/home/liuyc/Research/P02.Ts_change_research/figure/02.cmip6Result/1.4/', lower(mlabels.level), level.time1{exmNum});
    ...
    for mdlNum = mdlStart:mdlEnd
        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%
            %read data
            dradTrendPath = fullfile(esmPath, level.process3{7}); %/data1/liuyincheng/cmip6-process/amiexmNum980-2014/CESM2/Effect_trend
            danomTrendPath = fullfile(esmPath, level.process3{3}); %/data1/liuyincheng/cmip6-process/amiexmNum980-2014/CESM2/anomaly_trend
            ...
            tt = ['Level:', mlabels.level, ', Era: ', level.time1{exmNum}(1:end - 1), ', Model:', level.model2{mdlNum}];
            sgtt = sgtitle(headLineTxt, 'Fontsize', 14, 'Interpreter', 'none');
            figTitle = [level.time1{exmNum}(1:end - 1), '_', level.model2{mdlNum}, '_', mlabels.fileN1];
