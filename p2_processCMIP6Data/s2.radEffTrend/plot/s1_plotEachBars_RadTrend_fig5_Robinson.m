...
for exmNum = exm_left:exm_right
    ...
    mPath.uniOutput = fullfile('/home/liuyc/Research/P02.Ts_change_research/figure/proj2_cmip6Result/Radiative_effect/v0.7/fig2/', level.time1{exmNum});
    ...
    for mdlName = mdl_left:mdl_right
        ...
        ss = get(0, 'ScreenSize');
        h = figure('Position', [22 415 1413 524]);
        % set(h,'visible','off');
        ...
        for varNum = 1:f_row * f_col
            ...
            % end
            m_grid('tickdir','out','linewi',2);%,'backcolor',[.8 .8 .8]
        ...
        headLineTxt = {['Level:', mlabels.level, ', Era: ', level.time1{exmNum}(1:end - 1), ', Trend(year mean)'], ['Model:', level.model2{mdlName}, ', Ensemble: ', esm]};
        sgtt = sgtitle(headLineTxt, 'Fontsize', 14, 'Interpreter', 'none');
        figName = [level.time1{exmNum}(1:end - 1), '_', level.model2{mdlName}, '_', mlabels.fileN1, '_', esm];
        figPath = [mPath.Output, '/', figName, '.png'];
        saveas(gcf,figPath)
