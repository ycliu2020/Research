...
% LastEditTime : 2020-09-19 16:30:02
% LastEditors  : LYC
% Description  : 同时画时间序列和相关性分布图
% FilePath     : /code/p2_processCMIP6Data/s2.radEffTrend/timeSeriAnalysis/timeSeriANS_fig3Radtest_eachPoint.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 4:4%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    mPath.uniOutput1 = fullfile('/home/liuyc/Research/P02.Ts_change_research/figure/proj2_cmip6Result/TimeSeries_analysis/landMean_RadEffectAll_eachPoint', level.time1{exmNum}); %['dRTs_', lower(mlabels.level)],
    ...
    for mdlNum = 1:1%length(level.model2)
        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            % varUsed(:, :, :, 8) = dhFlux;
            % varUsed(:, :, :, 9) = dhFlux + dR_netSfc;

            % varNames = {'dR_{Ts}', 'dR_{cloud}', 'dR_{q}', 'dR_{ta}', 'dR_{albedo}', 'dR_{residual}', 'dR_{netSFC}', 'dR_{LH+SH}', 'dR_{netSFC+LH+SH}'};
            % yLabel = {'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2'};
            % varColor = {'#000000', '#4450A1', '#F08212', '#C2162E', '#90C64D', '#67C1EE', 'r', 'b', 'g'}; %黑色 深蓝 橘色 红色 浅绿 浅蓝
            ...
            varUsed(:, :, :, 6) = taEffect;
            varUsed(:, :, :, 7) = dR_cloud_sfc;
            varNames = {'dR_{Ts}', 'dR_{heating}', 'dR_{netSFC}', 'dR_{LH+SH}', 'dR_{ts+Rheating+dRnetSFC}', 'dR_{Ta}', 'dR_{cloud}'};
            yLabel = {'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2', 'Wm-2'};
            varColor = {'#000000', '#4450A1', '#F08212', '#C2162E', '#90C64D','#4EC5F1','#0df2c9'}; %黑色 深蓝 橘色 红色 浅绿 浅蓝
            ...
            % get the total points in land (lonLabel latLabel: the location of lat and lon)
            temp1 = varUsedYearly(:, :, 1, 1);
            pointsLabel = find(~isnan(temp1));
            pointsCount = numel(pointsLabel);
            [lonLabel latLabel] = find(~isnan(temp1));
            % save them in one var
            sizeVarUsedYearly = size(varUsedYearly);
            varUsedYearly_whole = zeros(pointsCount, sizeVarUsedYearly(3), sizeVar); %(ponits, time, dimVar)

            for pointsNum = 1:pointsCount
                varUsedYearly_whole(pointsNum, :, :) = squeeze(squeeze(varUsedYearly(lonLabel(pointsNum), latLabel(pointsNum), :, :)));
            end

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% plot each point
            for pointsNum = 1:pointsCount
                varUsedYearly_weightMean = squeeze(varUsedYearly_whole(pointsNum, :, :));
                ...
                latTxt=num2str(lat_f(latLabel(pointsNum)));
                lonTxt=num2str(lon_f(lonLabel(pointsNum)));
                title_txt = {['Level:', num2str(toaSfc{2}), ', Era: ', level.time1{exmNum}(1:end - 1)], ['Model:', level.model2{mdlNum} ', Ensemble: ', esmName{esmNum}], ['Lat: ',latTxt,' Lon: ', lonTxt, varNames{varNum}, ' & ', varNames{1}, ' cc=', num2str(cc_weightMean{2})]};
                ...
                % save figures
                figName = [level.time1{exmNum}(1:end - 1), '_', level.model2{mdlNum}, '_', esmName{esmNum},'_lat_',latTxt,'_lon_',lonTxt];
