...
% LastEditTime : 2021-04-02 09:58:04
% LastEditors  : Please set LastEditors
% Description  : 同时画时间序列和相关性分布图
% FilePath     : /code/p2_processCMIP6Data/s2.radEffTrend/timeSeriAnalysis/timeSeriANS_radClose_toa.m
%
%%---------------------------------------------------------
clc; clear; tic;
nowpath = pwd;
...
for exmNum = 2:4%1 mean amip 2000; 2 mean amip 1980;3 means ssp245, 4 means ssp370; 5 mean amip-hist 2000; 6 mean amip-hist 1980
    ...
    mPath.uniOutput = fullfile('/home/liuyc/Research/P02.Ts_change_research/figure/proj2_cmip6Result/v0.1/TimeSeries_analysis/radClosure/TOA', level.time1{exmNum}); %['dRTs_', lower(mlabels.level)],
    ...
    for mdlNum = 1:length(level.model2)
        ...
        for esmNum = specificNum:specificNum%1:1%length(esmName)% note that r1i1p1 sometime not the first folder
            ...
            % TotalEffect
            load([dradEffectPath, 'dradEffect_toa_clr.mat'])% 'totalEffect', 'wvlwEffect', 'wvswEffect', 'tsEffect', 'albEffect', 'husEffect', 'taEffect', 'tasEffect2', 'taOnlyEffect2', 'totalEffect', 'mainEffect'
            load([dradEffectPath, 'model_dradEffect.mat'])% 'dR_net_cld', 'l_rad', 's_rad', 'dR_net_clr', 'readme_realradEffect'
            load([dradEffectPath, 'dR_residual_clr_toa.mat'])% dR_residual_cld_sfc
            load([dradEffectPath, 'dR_residual_cld_toa.mat'])% dR_residual_cld_toa
            dR_net_cld_sfc = squeeze(dR_net_cld(:, :, :, 1));
            dR_net_cld_toa = squeeze(dR_net_cld(:, :, :, 2));

            dR_net_clr_sfc = squeeze(dR_net_clr(:, :, :, 1));
            dR_net_clr_toa = squeeze(dR_net_clr(:, :, :, 2));
            % vars tsEffect
            ...
            % 为了画图的美观, 拉伸定位坐标位置
            if exmNum==2
                timeNum(1)=timeYearly(1); % 对应1980
                timeNum(8)=timeYearly(end);% 对应2014
            elseif exmNum==1
                timeNum(5)=timeYearly(1); % 对应2000
                timeNum(8)=timeYearly(end); % 对应2014
            else
                timeNum(8)=timeYearly(1); % 对应2014
                timeNum(end)=timeYearly(end); % 对应2099
            end
            % fig set
